...
const endpoint =
  opts.endpoint || "https://8080-teal-nightingale-r7f5pbt6.ws-eu13.gitpod.io";
...
if (argument && fs.existsSync(argument)) {
  ...
  fetch(`${endpoint}/includes/api?url=${argument}`)
    .then((res) => {
      if (res.ok) {
        return res.json();
      } else {
  ...
  fetch(`${endpoint}/includes/get-api?code=${argument}`)
    .then((res) => {
      if (res.ok) {
        return res.json();
      } else {
